...
這些`dotnet ef`命令包含在 .NET Core SDK 中，但若要啟用您必須安裝`Microsoft.EntityFrameworkCore.Design`套件的命令。
...
|                   | 選項                   | 說明                                              |
|:------------------|:-------------------------|:---------------------------------------------------------|
| <nobr>    </nobr> |  `--connection <CONNECTION>`        | 資料庫的連接字串。 預設為或`AddDbContext` `OnConfiguring`中指定的值。 |
...
|                 | `--context-namespace <NAMESPACE>`        | 要用於所產生`DbContext`類別的命名空間。 注意：覆`--namespace`寫。                                 |
| `-f`            | `--force`                                | 覆寫現有檔案。                                                                                                                                                      |

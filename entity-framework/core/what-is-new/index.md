...
* 會針對支援的版本進行安全性與其他嚴重錯誤修補。 請一律使用給定版本的最新修補程式。 例如，針對 EF Core 2.1，使用 2.1. x 表示最高可用的 ' x '。
...
EF Core 發行與 [.NET Core 推出排程](https://github.com/dotnet/core/blob/main/roadmap.md) \(英文\) 一致。
...
下一個規劃的穩定版本為 **EF Core 6.0**，已排程于 **2021 年11月**。

遵循記載的[發行規劃](xref:core/what-is-new/release-planning)程式，建立[EF Core 6.0 的高階方案](xref:core/what-is-new/ef-core-6.0/plan)。
...
此外，當我們進行時，經常會將預覽推送至 NuGet。 請注意，預覽會延後每日組建，但會進行測試以使用對應的 ASP.NET Core 和 .NET Core preview。

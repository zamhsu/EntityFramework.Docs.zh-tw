...
擁有的實體類型可以與另一個自有實體類型共用相同的 .NET 類型，但因為僅依 .NET 類型並無法加以識別，所以必須從另一個實體類型進行導覽。 包含定義導覽的實體是擁有者。 查詢擁有者時，預設會包含擁有的類型。
...
- 依照慣例，產生 SQL 時，方法的名稱會用作函式的名稱 (在此案例中，為使用者定義函式)，但您可以在方法註冊期間覆寫該名稱與結構描述
- 目前只支援純量函式。
- 您必須在資料庫中建立對應函式。 EF Core 移轉將不負責加以建立。

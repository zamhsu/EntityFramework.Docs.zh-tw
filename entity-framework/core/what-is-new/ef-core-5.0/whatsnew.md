...
EF Core 5.0 是主要版本，也包含幾項 [重大變更](xref:core/what-is-new/ef-core-5.0/breaking-changes)，也就是 API 改進或可能對現有應用程式造成負面影響的行為變更。
...
EF Core 5.0 依照慣例將此辨識為多對多關聯性，並 `PostTag` 在資料庫中自動建立聯結資料表。 您可以查詢和更新資料，而不需要明確參考聯結資料表，以大幅簡化程式碼。 您仍然可以視需要明確地自訂和查詢聯結資料表。
...
EF Core 5.0 引進了一種簡單的方式，讓您透過新的方法設定記錄 `LogTo` 。 下列程式會將記錄訊息寫入主控台，包括 EF Core 產生的所有 SQL：
...
如需詳細資訊，請 [參閱已篩選包含的完整檔](xref:core/querying/related-data/eager#filtered-include)集。
...
實體類型通常會對應至資料表或視圖，因此 EF Core 會在查詢該型別時，取回資料表或視圖的內容。 EF Core 5.0 新增額外的對應選項，其中實體可以對應到 SQL 查詢 (稱為「定義查詢」 ) ，或對應至資料表值函數 (TVF) ：
...
在 EF Core 3.1 中，一對一關聯性的相依端點一律視為選擇性。 這在使用擁有的實體時最為明顯，因為所有擁有實體的資料行在資料庫中建立為可為 null，即使它們已在模型中設定為必要。

在 EF Core 5.0 中，擁有實體的導覽可以設定為必要的相依。 例如：
...
EF Core 5.0 引進 `AddDbContextFactory` 並 `AddPooledDbContextFactory` 註冊 factory，以在應用程式的相依性插入 (D.I. ) 容器中建立 DbCoNtext 實例; 當應用程式程式碼需要手動建立和處置內容實例時，這會很有用。
...
此時，可以使用 ASP.NET 核心控制器之類的應用程式服務來插入，並用 `IDbContextFactory<TContext>` 它來具現化內容實例：
...
相較于其他資料庫，SQLite 在其架構操作功能方面相當有限;例如，不支援從現有的資料表卸載資料行。 EF Core 5.0 藉由自動建立新的資料表、從舊的資料表複製資料、卸載舊的資料表，然後重新命名新的資料表，來解決這些限制。 這會「重建」資料表，並允許安全地套用先前不支援的遷移作業。
...
例如，下列範例會將資料 `Name` 行設定為在 SQL Server 上區分大小寫，而且在資料行上建立的任何索引都會據以運作：
...
* 改進 `DateTime` SQL Server (的翻譯 [`DateDiffWeek`](xref:Microsoft.EntityFrameworkCore.SqlServerDbFunctionsExtensions.DateDiffWeek%2A) ，例如 [`DateFromParts`](xref:Microsoft.EntityFrameworkCore.SqlServerDbFunctionsExtensions.DateFromParts%2A)) 。
...
* 已引進 Api 來控制 [交易儲存點](xref:core/saving/transactions#savepoints)。 此外，當呼叫且交易已在進行時，EF Core 會自動建立儲存點 `SaveChanges` ，並在失敗時回復。
...
* 新的 [`dotnet ef migrations list`](xref:core/cli/dotnet#dotnet-ef-migrations-list) 命令現在會顯示尚未套用至資料庫的遷移 ([`Get-Migration`](xref:core/cli/powershell#get-migration) 在封裝管理主控台) 中相同。
...
* 新 [`--no-onconfiguring`](xref:core/cli/dotnet#dotnet-ef-dbcontext-scaffold) 選項可以指示 EF Core `OnModelConfiguring` 在建立模型時排除。
...
* EF Core 命令列工具現在會自動將 `ASPNETCORE_ENVIRONMENT` _和_ `DOTNET_ENVIRONMENT` 環境變數設定為「開發」。 這可讓您在開發期間使用泛型主機與 ASP.NET Core 的使用體驗。

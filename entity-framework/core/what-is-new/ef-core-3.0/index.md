...
在 3.0 中，另一個大幅變更的設計層面，就是我們現在一律會針對每個 LINQ 查詢產生單一 SQL 陳述式。 在舊版中，我們通常會在某些情況下產生多個 SQL 陳述式，轉譯集合導覽屬性的 `Include()` 呼叫，以及轉譯遵循具有子查詢之特定模式的查詢。 雖然這在某些情況下很方便，而且對於 `Include()` 來說甚至可以協助避免透過網路傳送重複的資料，但實作起來很複雜，導致產生某些效率極低的行為 (N+1 個查詢)。 在某些情況下，跨多個查詢傳回的資料可能不一致。
...
EF Core 3.0 會利用幾個 [C# 8.0 中的新功能](/dotnet/csharp/whats-new/csharp-8) \(部分機器翻譯\)：
...
因此，我們決定移植最新版本的 EF 6，以在 .NET Core 3.0 上執行。

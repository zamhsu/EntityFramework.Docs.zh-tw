...
反向工程是基類庫實體類型類別的程式，以及以資料庫架構為基礎的 DbCoNtext 類別。 您可以使用 EF Core 套件管理員主控台（PMC）工具的 `Scaffold-DbContext` 命令，或 .NET 命令列介面（CLI）工具的 `dotnet ef dbcontext scaffold` 命令來執行此作業。
...
您如何加上引號和 escape 連接字串的方式，取決於您用來執行命令的 shell。 如需詳細資訊，請參閱您的命令介面檔。 例如，PowerShell 會要求您將 `$` 字元，但不能 `\`。
...
PMC 中的 `-Schemas` 參數和 CLI 中的 [`--schema`] 選項，可以用來包含架構中的每個資料表。
...
根據預設，資料表和資料行名稱已修正，以便更符合類型和屬性的 .NET 命名慣例。 在 PMC 中指定 `-UseDatabaseNames` 參數或在 CLI 中的 [`--use-database-names`] 選項，將會停用此行為，盡可能保留原始的資料庫名稱。 不正確 .NET 識別碼仍然會固定，而合成名稱（例如導覽屬性）仍會符合 .NET 命名慣例。

## <a name="fluent-api-or-data-annotations"></a>流暢的 API 或資料批註

實體類型預設會使用流暢的 API 來設定。 請盡可能指定 `-DataAnnotations` （PMC）或 `--data-annotations` （CLI），以改為使用資料批註。
...
根據預設，scaffold DbCoNtext 類別名稱會是以*內容*尾碼的資料庫名稱。 若要指定不同的帳戶，請在 CLI 中使用 PMC 和 `--context` 中的 `-Context`。
...
不過，更重要的變更並不是手動進行。 其中一個常見的工作流程是使用 `-Force` （PMC）或 `--force` （CLI），再次從資料庫反向工程模型，以覆寫現有的模型。

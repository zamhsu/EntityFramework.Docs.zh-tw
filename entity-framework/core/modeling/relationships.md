...
依照慣例，cascade delete 將會設定為*Cascade*必要的關聯性和*ClientSetNull*選擇性的關聯性。 *Cascade*表示相依實體也會一併刪除。 *ClientSetNull*方法不會載入記憶體的相依實體將會保留不變且必須以手動方式刪除，或更新為指向有效的主要實體。 載入到記憶體的實體，EF 核心會嘗試將外部索引鍵屬性設定為 null。

...
下列程式代碼清單顯示 `Blog` 與 `Post` 之間的一對多關聯性
...
* 如果相依實體包含名為 `<primary key property name>`、`<navigation property name><primary key property name>`或 `<principal entity name><primary key property name>` 的屬性，則會將它設定為外鍵。
...
雖然建議在相依實體類別中定義外鍵屬性，但它並不是必要的。 如果找不到任何外鍵屬性，將會以名稱 `<navigation property name><principal key property name>` 引進陰影外鍵屬性（如需詳細資訊，請參閱[陰影屬性](shadow-properties.md)）。
...
有兩個數據批註可用來設定關聯性、`[ForeignKey]` 和 `[InverseProperty]`。 這些可在 `System.ComponentModel.DataAnnotations.Schema` 命名空間中取得。

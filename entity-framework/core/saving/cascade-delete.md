...
就上述第二個動作而言，如果外部索引鍵值不可為 Null，將其設定為 Null 就會無效。 （不可為 null 的外鍵等同于必要的關聯性）。在這些情況下，EF Core 追蹤外鍵屬性已標示為 null，直到呼叫 SaveChanges 為止，此時會擲回例外狀況，因為變更無法保存至資料庫。 這與從資料庫收到條件約束違規類似。

...
提供者的註冊方式是在**entityFramework**區段的 provider 子區段底下包含**provider**元素。 提供者專案有兩個必要屬性:  
...
如需**DatabaseLogger**和註冊攔截器的詳細資訊, 請參閱[EF 6.1 的 blog 文章:開啟記錄而不重新](https://blog.oneunicorn.com/2014/02/09/ef-6-1-turning-on-logging-without-recompiling/)編譯。  
...
如果您未設定預設的連接 factory, Code First 會使用 SqlConnectionFactory, 指向`.\SQLEXPRESS`。 SqlConnectionFactory 也有一個可讓您覆寫連接字串部分的函式。 如果您想要使用以外`.\SQLEXPRESS`的 SQL Server 實例, 您可以使用此函數來設定伺服器。  

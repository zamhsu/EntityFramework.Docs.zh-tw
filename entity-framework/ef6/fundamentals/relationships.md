...
在 Entity Framework 中，實體可以透過關聯或關聯性與其他實體相關聯。 每個關聯性都包含兩個端，描述該關聯性中兩個實體的實體類型和類型的多重性（一個、零或一或多個）。 關聯性可能受到參考條件約束的規範，其中描述關聯性中的哪個 end 是主體角色，而後者是相依角色。
...
在*外鍵關聯*中，當您變更關聯性時，具有 `EntityState.Unchanged` 狀態之相依物件的狀態會變更為 [`EntityState.Modified`]。 在獨立的關聯性中，變更關聯性並不會更新相依物件的狀態。
...
- 將新物件指派給導覽屬性。 下列程式碼會建立課程和 `department`之間的關聯性。 如果物件附加至內容，則 `course` 也會加入至 `department.Courses` 集合，而 `course` 物件上對應的外鍵屬性會設定為部門的索引鍵屬性值。  
  ``` csharp
...
- 在實體集合中刪除或加入物件。 例如，您可以將 `Course` 類型的物件加入至 `department.Courses` 集合。 這種作業會建立特定**課程**和特定 `department`之間的關聯性。 如果物件附加至內容，則會將 [部門參考] 和 [**課程**] 物件上的外鍵屬性設定為適當的 `department`。  
  ``` csharp
  department.Courses.Add(newCourse);
  ```

- 藉由使用 `ChangeRelationshipState` 方法來變更兩個實體物件之間指定之關聯性的狀態。 當使用多層式應用程式和*獨立關聯*（無法與外鍵關聯一起使用）時，最常使用這個方法。 此外，若要使用此方法，您必須向下 `ObjectContext`，如下列範例所示。  
在下列範例中，講師和課程之間有多對多關聯性。 呼叫 `ChangeRelationshipState` 方法並傳遞 `EntityState.Added` 參數，可讓 `SchoolContext` 知道這兩個物件之間已加入關聯性：
  ``` csharp
